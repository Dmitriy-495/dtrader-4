...
const rootEnvPath = path.resolve(__dirname, '../../../../dtrader-4/.env');
...
if (!process.env.GATEIO_API_KEY || !process.env.GATEIO_API_SECRET) {
  console.log('⚠️  .env файл не найден или не содержит API ключей, используются значения по умолчанию');
}

// Логируем загрузку конфигурации
console.log('🔑 GATEIO_API_KEY:', process.env.GATEIO_API_KEY ? '✅ Настроен' : '❌ Не настроен');
console.log('🔑 GATEIO_API_SECRET:', process.env.GATEIO_API_SECRET ? '✅ Настроен' : '❌ Не настроен');
