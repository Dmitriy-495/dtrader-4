...
const rest_1 = require("./exchanges/gateio/rest");
// HTTP Server
const server = http_1.default.createServer((req, res) => {
    // Простой маршрутизатор с типами
    if (req.method === "GET" && req.url === "/api/status") {
        const response = { status: "DTrader-4 is running!" };
...
const wss = new ws_1.default.Server({ port: config_1.config.WS_PORT });
...
function shutdown(signal) {
    if (config_1.config.isShuttingDown)
        return;
    config_1.config.isShuttingDown = true;
    ...
    const shutdownTimeout = setTimeout(() => {
        console.log("⚠️  Таймаут ожидания закрытия серверов. Принудительное завершение.");
        process.exit(1);
    }, 5000);
...
async function main() {
    ...
    if (config_1.config.exchange.enabled) {
        console.log("🔄 Gate.io Exchange: Ключи API настроены");
        console.log(`🔑 API Key: ${config_1.config.exchange.apiKey ? "****" : "отсутствует"}`);
        console.log(`🔑 API Secret: ${config_1.config.exchange.secret ? "****" : "отсутствует"}`);
        console.log("💡 Готово к работе с Gate.io API через HTTP");
        // Создаем экземпляр REST соединения с API ключами
        const gateioRest = new rest_1.GateioRest(config_1.config.exchange.apiKey, config_1.config.exchange.secret);
        ...
        if (balance) {
            ...
            if (Array.isArray(balance) && balance.length > 0) {
                ...
                balance.slice(0, 3).forEach((asset) => {
    ...
    server.listen(config_1.config.PORT, () => {
        console.log(`🚀 DTrader-4 REST API запущен на http://localhost:${config_1.config.PORT}`);
        console.log(`📡 WebSocket сервер запущен на ws://localhost:${config_1.config.WS_PORT}`);
        if (config_1.config.exchange.enabled) {
